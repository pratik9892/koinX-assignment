...
const Trending = () => {
    ...
    const TrendingItem = ({src,title,percent,percentValue,slug}) => {
        return (
            <React.Fragment>
                ...
                className='flex items-start justify-between my-1 md:my-2 w-[349px] h-[28px] md:h-[28px] md:w-[379px] mx-auto cursor-pointer'>
  ...
  return (
    <div className='md:h-[225px] md:w-[427px] p-6 bg-[#FFFFFF] rounded-lg px-[10px] py-4 flex flex-col gap-2'>
        ...
        <div className='flex flex-col gap-2 w-[359px] h-[124px] md:w-[379px] mx-auto jus'>
            <TrendingItem slug={`${coin1?.item?.slug}`} src={coin1?.item?.large} title={coin1?.item?.name} percent={coin1?.item?.data?.price_change_percentage_24h?.usd.toFixed(2)}
            percentValue={coin1?.item?.data?.price_change_percentage_24h?.usd > 0 ? true : false}
            />
            <TrendingItem slug={`${coin2?.item?.slug}`} src={coin2?.item?.large} title={coin2?.item?.name} percent={coin2?.item?.data?.price_change_percentage_24h?.usd.toFixed(2)}
            percentValue={coin2?.item?.data?.price_change_percentage_24h?.usd > 0 ? true : false}
            />
            <TrendingItem slug={`${coin3?.item?.slug}`} src={coin3?.item?.large} title={coin3?.item?.name} percent={coin3?.item?.data?.price_change_percentage_24h?.usd.toFixed(2)}
